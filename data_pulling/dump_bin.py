...
class DumpDataBase:
    ...
    def _get_source_data(self, file_path: Path) -> pd.DataFrame:
        try:
            ...
            df = pd.read_feather(str(file_path.resolve()))
    ...
    def save_instruments(self, instruments_data: Union[list, pd.DataFrame]):
        ...
        if isinstance(instruments_data, pd.DataFrame):
            ...
            instruments_data[self.symbol_field_name] = instruments_data[
                self.symbol_field_name
            ].apply(lambda x: fname_to_code(str(x).lower()).upper())
        ...
        for field in self.get_dump_fields(_df.columns):
            ...
            if bin_path.exists() and self._mode == self.UPDATE_MODE:
                ...
                try:
                    ...
                    # arr = (
                    #     " ".join(
                    #         format(ord(letter), "b") for letter in _df[field].to_json()
                    #     ),
                    # )[0]
                    # dtype = arr.dtype
                    # arr = int(arr, base=2)
                    dater = pd.DataFrame([date_index], columns=["trades"])
                    dft = _df[field].reset_index(drop=True)

                    arr = pd.concat([dater, dft], axis=0).reset_index(drop=True)

                    arr.to_feather(str(bin_path.resolve()))
                    # arr.to_feather)
                # pa.feather.write_feather(arr, str(bin_path.resolve()))
                # arr.tofile(str(bin_path.resolve()))
...
class DumpDataUpdate(DumpDataBase):
    def __init__(
        ...
        self._new_calendar_list = self._old_calendar_list + sorted(
            filter(
                lambda x: try_get_name(x) > self._old_calendar_list[-1],
