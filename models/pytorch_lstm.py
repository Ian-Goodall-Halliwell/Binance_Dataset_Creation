...
class LSTM():
    ...
    def test_epoch(self, data_x, data_y):
        ...
        for i in tqdm(range(len(indices))[:: self.batch_size],position=0,leave=False):
    ...
    def fit(
        ...
        self.fitted = True
        pbar = tqdm(range(self.n_epochs),position=1,desc=f"Train MSE: {0}, Valid MSE: {0}")
        for step in pbar:
            ...
            pbar.set_description(f"Train MSE: {train_score}, Valid MSE: {val_score}")
    ...
    def predict(self, x_test, segment: Union[Text, slice] = "test"):
        if not self.fitted:
