def getfields(windows,rolling_windows):
    ...
    fields += ["Corr(RefBTC($close,0)/RefBTC($close,1), Log($volume/(Ref($volume, 1)+1e-12)+1), %d) - Corr(RefBTC($close,1)/RefBTC($close,2), Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % (d,d) for d in windows]
    names += ["CORRBTCV%d" % d for d in windows]
    fields += ["Corr(RefETH($close,0)/RefETH($close,1), Log($volume/(Ref($volume, 1)+1e-12)+1), %d) - Corr(RefETH($close,1)/RefETH($close,2), Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % (d,d) for d in windows]
    names += ["CORRETHV%d" % d for d in windows]
    fields += ["Corr(RefBTC($close,0)/RefBTC($close,1), $close/Ref($close, 1), %d) - Corr(RefBTC($close,1)/RefBTC($close,2), Ref($close,1)/Ref($close, 2), %d)" % (d,d) for d in windows]
    names += ["CORRBTC%d" % d for d in windows]
    fields += ["Corr(RefETH($close,0)/RefETH($close,1), $close/Ref($close, 1), %d) - Corr(RefETH($close,1)/RefETH($close,2), Ref($close,1)/Ref($close, 2), %d)" % (d,d) for d in windows]
    names += ["CORRETH%d" % d for d in windows]
    
    fields += ["Corr(Log(RefBTC($volume,0)/RefBTC($volume, 1)+1), Log($volume/(Ref($volume, 1)+1e-12)+1), %d) - Corr(Log(RefBTC($volume,1)/(RefBTC($volume, 2)+1e-12)+1), Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % (d,d) for d in windows]
    names += ["CORRBTCVV%d" % d for d in windows]
    fields += ["Corr(Log(RefETH($volume,0)/RefETH($volume, 1)+1), Log($volume/(Ref($volume, 1)+1e-12)+1), %d) - Corr(Log(RefETH($volume,1)/(RefETH($volume, 2)+1e-12)+1), Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % (d,d) for d in windows]
    ...
    # STOCHASTIC STUFF
    fields += [
        "(($close - Min($low,%d))/(Max($high,%d) - Min($low,%d)))" % (d, d, d)
        for d in windows
    ]
    names += ["STOCH%d" % d for d in windows]
    fields += [
        "Mean((($close - Min($low,%d))/(Max($high,%d) - Min($low,%d))),%d)"
        % (d, d, d, d)
        for d in windows
    ]
    ...
    fields += [
        "Corr($close/Ref($close,1), Log($volume/(Ref($volume, 1)+1e-12)+1), %d) - Corr(Ref($close,1)/Ref($close,2), Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % (d,d)
        for d in windows
    ]
    names += ["CORD%d" % d for d in windows]
    fields += ["Mean($close>Ref($close, 1), %d)/Mean(Ref($close,1)>Ref($close, 2),%d)" % (d,d) for d in windows]
    names += ["CNTP%d" % d for d in windows]
    fields += ["Mean($close<Ref($close, 1), %d)/Mean(Ref($close,1)<Ref($close, 2),%d)" % (d,d) for d in windows]
    names += ["CNTN%d" % d for d in windows]
    fields += [
        "(Mean($close>Ref($close, 1), %d)-Mean($close<Ref($close, 1), %d))/(Mean(Ref($close,1)<Ref($close, 2),%d) - Mean(Ref($close,1)>Ref($close, 2),%d))" % (d, d,d,d)
        for d in windows
    ]
    ...
    fields += ["Mean(Log($volume/(Ref($volume, 1)+1e-12)+1)/Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % d for d in windows]
    names += ["VMA%d" % d for d in windows]
    fields += ["Std(Log($volume/(Ref($volume, 1)+1e-12)+1)/Log(Ref($volume,1)/(Ref($volume, 2)+1e-12)+1), %d)" % d for d in windows]
